...
const read = async (req, res) => {
    try {
        // Use COALESCE to support databases with "nombre" or legacy "nombre_completo"
        const [rows] = await db.promise().execute(
            // DB has `nombre_completo` and `fecha_registro`; provide compatible aliases used by PHP API
            'SELECT id, nombre_completo AS nombre, email, NULL AS rol, activo, fecha_registro AS fecha_creacion FROM usuarios'
        );
...
const get = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await db.promise().execute(
            'SELECT id, nombre_completo AS nombre, email, NULL AS rol, activo, fecha_registro AS fecha_creacion FROM usuarios WHERE id = ?',
            [id]
        );
