...
if (!empty($data->id)) {
    $query = "UPDATE herramientas SET nombre=:nombre, empresa_id=:empresa_id, tipo=:tipo, estado=:estado, ultimaInspeccion=:ultimaInspeccion WHERE id=:id";
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(array("message" => "Herramienta actualizada correctamente."));
    } else {
        http_response_code(503);
        echo json_encode(array("message" => "No se pudo actualizar la herramienta."));
    }
} else {
    http_response_code(400);
    echo json_encode(array("message" => "ID de herramienta no proporcionado."));
}
?>